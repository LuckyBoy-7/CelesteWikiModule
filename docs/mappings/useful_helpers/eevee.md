...
* [Eevee Helper 文档](https://gamebanana.com/mods/53765){:target="_blank"}
* [Eevee Helper Github](https://github.com/CommunalHelper/EeveeHelper){:target="_blank"}
* [AfterDawn 的 eevee 教程](../../assets/mappings/useful_helpers/eevee/AfterDawn的eevee教程.docx)(群文件里有更新版本)
* [底龙 的 eevee 教程](https://uddrg.notion.site/UnderDragon-s-Partial-Wiki-2737f4f27e63808582b3f0689163d8f9?p=2737f4f27e6380f4bcd1c7002f2e692e&pm=s){:target="_blank"}
...
#### [筛选同类实体中的特定对象](https://www.bilibili.com/video/BV1bw8EzfErU/){:target="_blank"}
...
* Contain Mode([可以搭配视频感受下](https://www.bilibili.com/video/BV1kR8jzyExm){:target="_blank"}): 
    * 一次更新:
...
* ContainFlag: 不管是一次更新还是多次更新, ContainFlag 永远都是添加实体/更新的必要条件, 也就是这个 Flag 必须存在才会更新(除非你这一栏留空, 此时容器不受 Flag 影响)
    * 当 ContainFlag 不为空, 且 Contain Mode 为一次更新类型时, ContainFlag 将表示为一个`使用开关`, 也就是表示容器是否生效, 但它仍然会影响容器添加实体的操作, 也就是上述的更新操作(你可以观看[视频](https://www.bilibili.com/video/BV1kR8jzyEhv){:target="_blank"}来理解这个过程)
...
* Force Standard Behavior: 是否适配容器内的一些特殊实体, 如果勾选则会在某些操作时统一使用一个行为, 即不适配, 如果不勾选则会适配, 比如在[视频](https://www.bilibili.com/video/BV1kR8jzyEwV){:target="_blank"}中我们把水搬走的时候如果不做适配就会出问题, 但有时候不做适配产生的奇异搞笑的效果也可能是我们想要的(
...
* [尘埃绕着行星转的效果](https://www.bilibili.com/video/BV1rR8jzyEQa){:target="_blank"}: 搭配后续提到的 `Attached Container` 食用
...
* Static Fit: 简单来说如果你开起了 Fit Contained, 那么如果容器内的实体位置发生变化, 硬币门在移动时会补偿这些变化, 所以你会看到容器歪歪扭扭的过去了, 此时如果开启 Static Fit, 则容器会强制沿着路线移动, 你可以观看[视频](https://www.bilibili.com/video/BV1wj8DzzE5k/){:target="_blank"}来感受区别
* [Easing](https://easings.net/){:target="_blank"}: 将均匀变化的时间映射到不均匀变化的运动上, 以实现各种先快后慢, 先慢后快的运动 
* 其他基本上都是讲过的或者看名字就知道功能的这里就不讲了(

这个实体可以用来做(看你想象力)

* [压扁机关](https://www.bilibili.com/video/BV1YB8XzAEbn/){:target="_blank"}
* 实体突然出现, 改变位置之类的
...
为容器内实体添加抛接物的特性, 先来看看[西西弗斯](https://www.bilibili.com/video/BV13iLGz3ELu){:target="_blank"}(
...
你可以观看[视频](https://www.bilibili.com/video/BV1SchwzEEYJ/){:target="_blank"}来简单感受下他们是如何运作的
