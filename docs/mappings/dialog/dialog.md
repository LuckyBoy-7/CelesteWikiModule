...
接着你就可以在该目录下创建[各种语言的`.txt`](https://github.com/EverestAPI/Resources/wiki/Adding-Custom-Dialogue#setting-up-the-dialogue-file)
...
- 重复的 Dialog ID 后加载的会覆盖先加载的, 把这个特性当作技巧的话可以用来替换官图文本(同名的 Dialog ID 后加载的会覆盖先加载的,
  如 [FunnyDialog](https://www.bilibili.com/video/BV1Pz421i7SZ))
