...
蔚蓝的每个实体都有个Depth(深度)属性, 越深的物体越先更新, 也越先绘制, 如果两个实体Depth相同则先加载的先更新(在Loenn里对应的应该是对象摆放的先后顺序), 正是因为更新顺序不同, 所以它有时会导致一些奇妙的特性或是细微的差异(例如你想让刺吸附在红绿灯上, 但现在有两个重叠的红绿灯, 则先加载的先把刺占用)
...
当你按着跳的时候, 如果你的y方向速度范围在`[-40, 40]px/s`之间(也就是刚跳起和刚落下那一小段)的话, 重力加速度会减半, 比如正常来说`g`是`15px/s*f`,
...
- 提供了一定对准
- 增大了一定容错
- 让一些操作成为可能, 例如对于一个`7`字形的结构, 通过缓降你扒在左上角是可以轻易滑倒右下角上的砖上的, 如果没有缓降, 就会比较麻烦(`cp`)或者比较极限(
  不缓降直直的滑下去)
